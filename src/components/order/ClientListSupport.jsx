...
import { supportDelete, supports } from "../../services/supportService";
import { convertIntToArrayInit } from "../../helpers/GetArray";
import { Loading } from "../../layout/Loading";

export const ClientListSupport = () => {
  ...
  const onloading = async () => {
    setLoading(true);
    var request = await supports(params);
    if (request != null) {
  ...
  const removeSupport = async (e, SupportId) => {
    ...
    Swal.fire({
      title: "Are you sure?",
      text: "You want remove this report!",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        const request = await supportDelete(SupportId);
        if (request.success) {
          await onloading();
        }
      }
              ...
              <tbody>
                {response.data?.map((item, indice) => (
                  ...
                  >
                    ...
                    <td className="py-2 py-md-5">{item.contactLabel}</td>
